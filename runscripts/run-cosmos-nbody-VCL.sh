...
export QRACK_QPAGER_DEVICES_HOST_POINTER=8.0,0.1
...
export QRACK_OCL_DEFAULT_DEVICE=5
export QRACK_QPAGER_DEVICES=16.0
./vclrun ./benchmarks --optimal --layer-qtensornetwork --measure-shots=10000 -m=30 --single --samples=1 --measure-output=/var/log/qrack/"measured_cosmos_$(date +%F_%R_%S).log" test_qft_cosmology
...
export QRACK_QPAGER_DEVICES=8.0,8.1,8.3,8.5,8.6,8.7
./vclrun ./benchmarks --optimal --layer-qtensornetwork --measure-shots=10000 -m=32 --single --samples=1 --measure-output=/var/log/qrack/"measured_cosmos_$(date +%F_%R_%S).log" test_qft_cosmology 
...
# M40 24GB legacy run
export QRACK_OCL_DEFAULT_DEVICE=2
export QRACK_QPAGER_DEVICES=12.0
./vclrun ./benchmarks --optimal --layer-qtensornetwork --measure-shots=10000 -m=29 --single --samples=1 --measure-output=/var/log/qrack/"measured_cosmos_$(date +%F_%R_%S).log" test_qft_cosmology 
